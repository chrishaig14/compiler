...
SymbolInfo w_finfo(VectorOfTypes parameter_types, TypeNode return_type) {
    FunctionInfo finfo(parameter_types, return_type);
    SymbolInfo ginfo(finfo);
    return ginfo;
}

SymbolInfo w_sinfo(std::string type) {
    ObjectInfo sinfo(i_type(type, {}));
    SymbolInfo ginfo(sinfo);
    return ginfo;
}

FunctionInfo f_info(VectorOfTypes parameter_types, TypeNode return_type) {
    return FunctionInfo(parameter_types, return_type);
}
...
SecondPass::SecondPass(SymbolTable* globals, ClassTable* class_table) {
    ...
    this->class_table->set("Integer", ClassInfo(MapStringToSimple(), std::map<std::string, FunctionInfo>()));
...
SemanticInfo SecondPass::analyze(FunctionNode& n) {
    ...
    for (int i = 0; i < n.parameter_names.size(); i++) {
        ObjectInfo object_info(n.parameter_types[i]);
        SymbolInfo sinfo(object_info);
        this->scope->set(n.parameter_names[i], sinfo);
    }
    this->scope->set("__return__", wrap_simple_info(ObjectInfo(n.return_type)));
...
SemanticInfo SecondPass::analyze(IdentifierNode& n) {
    ...
    SemanticInfo semantic_info;
    semantic_info.symbol_info = new SymbolInfo(this->scope->get(n.name));
...
SemanticInfo SecondPass::analyze(DeclarationNode& n) {
    ...
    if (n.expression != nullptr and n.type != nullptr) {
        ...
        if (!equal(wrap_simple_info(ObjectInfo(*n.type)), *expression_info.symbol_info)) {
    ...
    this->scope->set(n.identifier, wrap_simple_info(ObjectInfo(*n.type)));
    return semantic_info;
}
...
SemanticInfo SecondPass::analyze(MemberNode& n) {
    ...
    if (class_info.fields.count(n.child) == 1) {
        // It's a field
        semantic_info.symbol_info = new SymbolInfo(class_info.fields[n.child]);
        return semantic_info;
    }
...
SemanticInfo SecondPass::analyze(BinopNode& n) {
    ...
    SemanticInfo semantic_info;
    semantic_info.symbol_info = new SymbolInfo(ObjectInfo(TypeNode("Integer", {})));
...
SemanticInfo SecondPass::analyze(ClassNode& n) {
    for (int i = 0; i < n.methods.size(); i++) {
        ...
        TypeNode type_node(n.name, {});
        ObjectInfo simple_info(type_node);
        SymbolInfo this_info(simple_info);
...
SemanticInfo SecondPass::analyze(AstNode& n) {
    switch (n.type) {
        ...
        case AstType::NUMBER: {
            SemanticInfo semantic_info;
            semantic_info.symbol_info = new SymbolInfo(w_sinfo("Integer"));
            return semantic_info;
        }
        case AstType::STRING: {
            SemanticInfo semantic_info;
            semantic_info.symbol_info = new SymbolInfo(w_sinfo("String"));
            return semantic_info;
        }
        case AstType::CALL:
