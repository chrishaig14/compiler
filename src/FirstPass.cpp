...
void FirstPass::analyze(ClassNode& node) {
    ...
    for (auto f: node.fields) {
        class_info.fields[f.identifier] = ObjectInfo(*f.type);
    }
    for (auto m: node.methods) {
        class_info.methods[m.name] = FunctionInfo(m.parameter_types, m.return_type);
    }
    this->class_table.set(node.name, class_info);
}

void FirstPass::analyze(FunctionNode& node) {
    FunctionInfo function_info(node.parameter_types, node.return_type);
    SymbolInfo ginfo(function_info);
...
FunctionInfo::FunctionInfo(std::vector<TypeNode> parameter_types, TypeNode return_type) {
    for (auto n: parameter_types) {
        this->parameter_types.push_back(ObjectInfo(n));
    }
        this->return_type = ObjectInfo(return_type);
}
...
ObjectInfo::ObjectInfo(TypeNode n) {
    ...
    for (auto t: n.type_parameters) {
        this->type_parameters.push_back(ObjectInfo(t));
    }
}
