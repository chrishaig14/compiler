...
i_fun(std::string name, VectorOfStrings parameter_names, VectorOfTypes parameter_types, TypeNode return_type,
      VectorOfNodes body) {
    return FunctionNode(name, parameter_names, parameter_types, return_type, body);
}

DeclarationNode i_decl(std::string name, AstNode* expression) {
    return DeclarationNode(name, nullptr, expression);
}

DeclarationNode i_decl_type(std::string name, TypeNode* type, AstNode* expression) {
    return DeclarationNode(name, type, expression);
}
...
ClassNode i_class(std::string name, VectorOfStrings template_parameters,
    ...
    return ClassNode(name, template_parameters, fields, methods);
}
...
TypeNode i_type(std::string name, VectorOfTypes type_parameters) {
    return TypeNode(name, type_parameters);
}
...
bool only_one_null(void* a, void* b) { return a != b && (a == nullptr || b == nullptr); }

bool equal(DeclarationNode a, DeclarationNode b) {
    if (only_one_null(a.expression, b.expression)) return false;
    if (only_one_null(a.type, b.type)) return false;
    bool eq_types = both_null(a.type, b.type) || equal(*a.type, *b.type);
    bool eq_exps = both_null(a.expression, b.expression) || equal(*a.expression, *b.expression);
    return a.identifier == b.identifier && eq_types && eq_exps;
}
