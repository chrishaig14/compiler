...
FunctionCallNumArgs::FunctionCallNumArgs(const sem::TypeFunction* type, const ast::ExpNode& node) : Error(ErrorType::function_call_num_args), start(node.start), end(node.end), type(type) {
}
