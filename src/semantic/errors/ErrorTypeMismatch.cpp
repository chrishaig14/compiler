...
ErrorTypeMismatch::ErrorTypeMismatch(const ast::Type& expected, const ast::Node& value_node, const Entity& actual)
        : expected(expected), value_node(value_node), actual(actual.clone()) {
}
...
bool ErrorTypeMismatch::equal(const Error& other) const {
    ...
    bool act = *o.actual == *this->actual &&
               (*((Value&) *o.actual).type == *((Value&) *this->actual).type);
