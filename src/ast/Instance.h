...
public:
    std::string id;
    UObjectType base_type;
    std::unordered_map<std::string, UFunctionNode> methods;
    Instance(const std::string& id, UObjectType base_type,
             std::unordered_map<std::string, UFunctionNode> methods, TextPosition start, TextPosition end);
