...
For::For(const std::string& var, UNode& exp, std::unique_ptr<ast::Block>& body, TextPosition start,
         TextPosition end) : Node(NodeType::FORLOOP, start, end), var(var), exp(std::move(exp)),
                                     body(std::move(body)) {
    // assert(exp != nullptr);
    // assert(body != nullptr);
}

bool For::equal(const Node& x) const {
